# Modified by bartoszp1992 in 2024: changed project name to flui(flashlight UI); removed links to original repos; other changes

flui User Manual
-------------------

flui is an open-source firmware for flashlights originally by ToyKeeper, distributed under
the terms of the GPL v3.

sources:
https://github.com/bartoszp1992/flui
...
flui's ramping mode uses a smooth ramp or a stepped ramp, depending
...
While the light is on, a few actions are available:
  ...
  - `10H`: Ramp extras config menu.
    ...
    - Item 4: Configure Advanced UI's turbo style:  
              ...
              1: flashlight firmware 1 style.  `Ramp -> 2C` goes to full power.  
              2: flashlight firmware 2 style.  `Ramp -> 2C` goes to ceiling,
...
flui has several other modes too.  To access these, press the button
...
Click 3 times from Off to access flui's blinky / utility modes.  This
...
flui includes a few extra modes for a variety of purposes:
...
several formats:
 ...
 - `MODEL-YYYY-MM-DD-SINCE-DIRTY`
   flashlight firmware 2 from 2023-12 or later.  "SINCE" and "DIRTY" may be omitted.
 ...
 - `NNNN-YYYY-MM-DD`
   flashlight firmware 2 from 2023-05 or later.  
 ...
 - `YYYYMMDDNNNN`
   flashlight firmware 2 from 2023-05 or earlier.  
   It's a build date and model number.

- `YYYYMMDD`
   This is an old flashlight firmware 1 version.  It's just a build date.  
...
flui includes low voltage protection (LVP) and thermal regulation.
...
This is a table of all button mappings in flui, in one place:
