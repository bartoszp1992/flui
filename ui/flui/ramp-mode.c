// ramp-mode.c: Ramping functions for flui.
...
uint8_t steady_state(Event event, uint16_t arg) {
    ...
    uint8_t turbo_level;
        ...
        #endif
        // 0 = no turbo
        // 1 = flashlight firmware 1 direct to turbo
        // 2 = flashlight firmware 2 direct to ceiling, or turbo if already at ceiling
    ...
    #elif defined(USE_2C_MAX_TURBO)  // flashlight firmware 1 style always
        // simple UI: to/from ceiling
        // full UI: to/from turbo (flashlight firmware 1 behavior)
        #ifdef USE_SIMPLE_UI
        if (cfg.simple_ui_active) turbo_level = mode_max;
        else
        #endif
        turbo_level = MAX_LEVEL;
    #else  // flashlight firmware 2 style always
        // simple UI: to/from ceiling
...
void ramp_extras_config_save(uint8_t step, uint8_t value) {
    ...
    #ifdef USE_2C_STYLE_CONFIG
    // item 4: flashlight firmware 1 2C turbo, or flashlight firmware 2 2C ceiling?
    // 1 = flashlight firmware 1, 2C turbo
    // 2+ = flashlight firmware 2, 2C ceiling
